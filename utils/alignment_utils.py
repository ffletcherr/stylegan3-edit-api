...
def get_rotation_from_eyes(
    left_eye_unaligned, right_eye_unaligned, left_eye_aligned, right_eye_aligned
):
...
def get_alignment_positions(
    dlib_image, detector, predictor, eyes_distance_only: bool = True
):
...
def get_fixed_cropping_transformation(c, x):
    ...
    d_ver = np.array([0, d])
    quad = np.stack(
        [c - d_hor - d_ver, c - d_hor + d_ver, c + d_hor + d_ver, c + d_hor - d_ver]
    )
    qsize = np.hypot(*x) * 2
...
def get_stylegan_transform(
    ...
    rotation_angle = get_rotation_from_eyes(
        left_eye_unaligned=unaligned_left_eye,
